...
class WeChatPublisher:
    ...
    def get_cover_for_article(self, title: str, platform: str = "bilibili") -> str:
        """根据文章标题和平台获取对应的封面图片
        ...
        # 构建covers目录路径
        covers_dir = os.path.expanduser(f"~/video-downloads/{platform}/covers/")
        
        # 策略1：精确匹配（平台_标题_cover.jpg）
        exact_match = os.path.join(covers_dir, f"{platform}_{safe_title}_cover.jpg")
        if os.path.exists(exact_match):
            print(f"✅ 找到精确匹配的封面: {exact_match}")
            return exact_match
        
        # 策略2：模糊匹配（平台_标题*_cover.jpg）
        pattern = os.path.join(covers_dir, f"{platform}_{safe_title}*_cover.jpg")
        matches = glob.glob(pattern)
        if matches:
            # 返回最新的文件
            latest_match = max(matches, key=os.path.getmtime)
            print(f"✅ 找到模糊匹配的封面: {latest_match}")
            return latest_match
        
        # 策略3：使用默认封面
        default_cover = os.path.expanduser("~/skills/wechat_article_skills/wechat-draft-publisher/cover.png")
        print(f"⚠️  未找到对应封面，使用默认封面: {default_cover}")
