...
5. **检查封面图**
   - 查找 `cover.png`
   - 如缺失则用 ffmpeg 从视频截取一帧

6. **⚠️ 重要：必须用完整 HTML 内容**
   - ❌ 禁止：直接写 `<h2>标题</h2><p>内容</p>` 这种假内容
   - ✅ 必须：用 text-visualizer 生成的完整 HTML 文件（如 ~/video-downloads/douyin/html/*.html）
   - 正确流程：delegate_task → text-visualizer → read_file 完整 HTML → publisher
   - 踩坑记录：2026-05-31 曾因用假内容测试导致草稿只有一句话！
